...
namespace Smooth
{
    ...
    public class SmoothSync : NetworkBehaviour
	{
        ...
        public bool hasAuthorityOrUnownedOnServer {
            get {
                return netID.hasAuthority || (NetworkServer.active && netID.connectionToClient == null);
            }
        }
...
#region Sync Properties
        ...
        void SendStateToNonOwners(MessageBase state)
        {
            foreach (var kv in netID.observers)
            {
                ...
                if (conn != null && (transformSource == TransformSource.Server || conn != netID.clientAuthorityOwner) && conn.hostId != -1 && conn.isReady)
                {
